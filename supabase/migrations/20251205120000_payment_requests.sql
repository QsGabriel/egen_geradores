...
CREATE TABLE IF NOT EXISTS payment_requests (
  ...
  tipo_solicitacao TEXT NOT NULL CHECK (tipo_solicitacao IN ('PAGAMENTO', 'REEMBOLSO', 'ADIANTAMENTO')),
  documento_numero TEXT,
  fornecedor TEXT NOT NULL,
  cpf_cnpj TEXT,
  valor_total DECIMAL(15, 2) NOT NULL,
  forma_pagamento TEXT NOT NULL CHECK (forma_pagamento IN ('PIX', 'DINHEIRO', 'BOLETO', 'CAJU', 'SOLIDES')),
...
COMMENT ON COLUMN payment_requests.tipo_solicitacao IS 'Tipo: PAGAMENTO, REEMBOLSO, ADIANTAMENTO';
...
COMMENT ON COLUMN payment_requests.forma_pagamento IS 'Forma: PIX, DINHEIRO, BOLETO, CAJU, SOLIDES';
...
COMMENT ON COLUMN payment_requests.autorizado_por IS 'Nome de quem autorizou (obrigatório)';
